...
function serialData(data, port) {
	...
	//console.log('got newline from serial: '+data); // Use this to debug incoming data

	// handle M105
 	if (data.indexOf('ok T:') == 0 || data.indexOf('T:') == 0) {
 		emitToPortSockets(port, 'tempStatus', data);
		//emitToPortSockets(port, 'serialRead', {'line':'<span style="color: red;">Temp: '+data+'</span>'});
		if (data.indexOf('T:') == 0)
			return;
 	}
	...
	if (data.indexOf('ok') == 0) {
		...
		// Print nothing
		//emitToPortSockets(port, 'serialRead', {'line':'<span style="color: yellow;">RESP: '+data+'</span>'});
